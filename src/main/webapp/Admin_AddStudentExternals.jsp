...
<form action="AddStudentExternals/Post" method="post">
    <h2>View Regsitered Students</h2>
    <input type="hidden" value="${cid}" name="cid">
    <input type="hidden" value="${sem}" name="sem">
    <input type="hidden" value="${ay}" name="ay">
  <table>
    <tr> 
      <th>Student id</th>
      <th>Student name</th>
      <th>Section</th>
      <th>Give External Marks(Out of 40)</th>
    </tr> 
    ...
    <c:forEach items="${scm}" var="slist" varStatus="status" > 
        <tr> 
          ...
          <c:if test="${slist.studentExternals == -1 }">
          <td><input type="number" max=60 min=1 name="${status.index}"></td> 
          </c:if>
          <c:if test="${slist.studentExternals != -1 }">
          <td><c:out value="${slist.studentExternals}"></c:out> (Already Posted)</td>
          </c:if>
        </tr> 
    </c:forEach> 
  </table>
  <div class="button-container">
            <input type="submit" value="Post Externals">
        </div>
  </form>
</div>
</c:if>
